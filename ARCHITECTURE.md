...
- Slot unlocks are the persistent progression flag: `state.minions.unlockedSlots` records which minion slots are permanently unlocked, and each slot's first free summon is granted by its typed `unlockMinionSlot` Soul Catcher effect. Repeat purchases after a Soul Catcher reset grant XP only. The save keeps the roster, permanent unlocks, and per-slot growth state; it does not store the legacy `unlockedEver`/paid-summon counters. Layer membership comes from the authored layer registry: Layer 1 includes `SC-MINION-01`, and Layers 2/3 each include their slot node. The published Layer 1 to 2 threshold remains 84,292 XP. Layer 3 uses seeded, authored effects with exponential purchase costs; `SC-65` unlocks Epic Soul drops from Epic enemies.
