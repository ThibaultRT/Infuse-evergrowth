...
export class HeroView extends AnimatedHumanoidView {
  ...
  private orbitWeapons: Partial<Record<'orbit1' | 'orbit2' | 'orbit3', THREE.Object3D>> = {};
  private orbitFlights: Partial<Record<'orbit1' | 'orbit2' | 'orbit3', { target: THREE.Vector3; progress: number }>> = {};
  ...
  update(dt: number, moving: boolean, active = true): void {
    ...
    if (this.attackTime > 0) this.biasLimbTowardTarget(progress);
  ...
  private updateOrbits(dt: number): void {
    ...
    for (const [slot, weapon] of Object.entries(this.orbitWeapons) as ['orbit1' | 'orbit2' | 'orbit3', THREE.Object3D][]) {
      const index = weapon.userData.orbitIndex as number; const angle = time * 1.4 + index * Math.PI * 2 / 3;
