...
export function renderSoulCatcher(nodes: SoulNode[], edges: [string, string][], revealed: (id: string) => boolean, canPurchase: (id: string) => boolean, selectedId: string | null): void {
  ...
  ui.soulNodes.innerHTML = nodes.map((node) => { const [x,y] = position(node), isRevealed = revealed(node.id), level = save.soulCatcher.nodeLevels[node.id] ?? 0, purchasable = canPurchase(node.id); return `<button type="button" class="soul-node ${isRevealed ? 'revealed' : 'mystery'} ${level ? 'purchased' : ''} ${purchasable ? 'purchasable' : ''} ${selectedId === node.id ? 'selected' : ''}" style="left:${x}px;top:${y}px" data-soul-node="${isRevealed ? node.id : ''}" aria-label="${isRevealed ? `${node.name}, level ${level}${purchasable ? ', upgrade available' : ''}` : 'Mystery node'}">${isRevealed ? `<span class="soul-node-icon">${soulIcon(node.cost.soulType)}</span>${purchasable ? '<span class="soul-upgrade" aria-hidden="true">↑</span>' : ''}<span class="soul-level">${level}/${node.maxLevel}</span>` : '?'}</button>`; }).join('');
